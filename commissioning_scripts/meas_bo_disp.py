...
class DispMeas:
    ...
    def run_meas(self, deltaf=0):
        ...
        self.rf.set_frequency(self.freq0+deltaf)
        print('Getting orbit ...')
        self.orbp = self.get_orb()
        self.freqp = self.rf.frequency
        print('Applying Delta - {:.6f} kHz'.format(deltaf*1e-3))
        self.rf.set_frequency(self.freq0-deltaf)
        print('Getting orbit ...')
        self.orbm = self.get_orb()
        self.freqm = self.rf.frequency
        print('Setting initial frequency ...')
        self.rf.set_frequency(self.freq0)
        self.calc_disp()
        print('Finished!')
